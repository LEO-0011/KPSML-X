...
async def _ytdl(client, message, isLeech=False, sameDir=None, bulk=[]):
    ...
    # FIXED: Improved cookie handling with multiple fallback paths
    options = {
        'usenetrc': True,
        'cookiefile': 'cookies.txt'  # Default path
    }
    ...
    cookie_paths = [
        ...
        '/usr/src/app/cookies.txt',  # Common Docker path
        'bot/cookies.txt',       # Bot subdirectory
    ]
    
    cookie_file_found = False
    for cookie_path in cookie_paths:
        if ospath_exists(cookie_path):
            options['cookiefile'] = cookie_path
            cookie_file_found = True
            LOGGER.info(f"Found cookies file at: {cookie_path}")
            break
    
    if not cookie_file_found:
        LOGGER.warning("No cookies.txt file found! YouTube downloads may fail.")
    ...
    # Override cookiefile if specified in options
    if 'cookiefile' not in options or not ospath_exists(options['cookiefile']):
        for cookie_path in cookie_paths:
            if ospath_exists(cookie_path):
                options['cookiefile'] = cookie_path
                break
    ...
    try:
        ...
        # Enhanced error message for cookie-related issues
        if 'Sign in to confirm' in msg or 'bot' in msg.lower() or 'cookies' in msg.lower():
            error_msg = f'{tag} YouTube requires authentication!\n\n'
            error_msg += '❌ Cookie file issue detected.\n\n'
            error_msg += '📋 To fix this:\n'
            error_msg += '1. Export YouTube cookies using browser extension\n'
            error_msg += '2. Save as cookies.txt in bot root directory\n'
            error_msg += '3. Make sure the file is in Netscape format\n\n'
            error_msg += f'🔍 Searched paths: {", ".join(cookie_paths)}\n'
            error_msg += f'✅ Current cookiefile: {options.get("cookiefile", "None")}\n\n'
            error_msg += f'Original error: {msg}'
            await sendMessage(message, error_msg)
        else:
